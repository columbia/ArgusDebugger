    ...
    if (new_invoke->is_begin()) {
        ...
        for (rit = dispatch_block_invoke_begin_list.rbegin(); 
            ...
            if (invokebegin->get_tid() == new_invoke->get_tid()
                    && invokebegin->get_ctxt() == new_invoke->get_ctxt()) {
...
{
    uint64_t q_id, ctxt, func, dc_flags, tid, coreid;
    std::string procname = "";
    if (!(iss >> std::hex >> q_id >> ctxt >> func >> dc_flags >> tid >> coreid))
        return false;
    if (!getline(iss >> std::ws, procname) || !procname.size())
        procname = "";

    BlockEnqueueEvent *new_enqueue = new BlockEnqueueEvent(abstime, opname,
                tid, q_id, ctxt, func, dc_flags, coreid, procname);
        ...
        std::istringstream &iss)
{
    ...
    if (!getline(iss >> std::ws, procname) || !procname.size())
        procname = "";
	bool begin = _begin == 1 ? true: false;

    BlockInvokeEvent *new_invoke = new BlockInvokeEvent(abstime, opname,
            tid, func, _ctxt, begin, coreid, procname);
